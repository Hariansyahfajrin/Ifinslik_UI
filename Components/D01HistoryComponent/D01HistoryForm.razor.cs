...
{
	public partial class D01HistoryForm
	{
		#region Service
		...
		#region Field
		public D01HistoryModel row = new();
		#endregion
		...
		{
			Loading.Show();
			row = await D01HistoryService.GetRowByID(ID) ?? new();
		...
		private async void OnSubmit()
		{
			...
			if (ID == null)
			{
				var res = await D01HistoryService.Insert(row);

				Loading.Close();

				if (res?.Data != null)
				{
					NavigationManager.NavigateTo($"/inquiry/datadebiturperoranganhistory/{FormTransactionHistoryID}/debiturhistory01/{res.Data.ID}", true);
				}
			}
			#endregion
			...
			{
				await D01HistoryService.UpdateByID(row);
			}
			#endregion
		...
		private void Back()
		{
			NavigationManager.NavigateTo($"/inquiry/datadebiturperoranganhistory/{FormTransactionHistoryID}");
		}
		#endregion
