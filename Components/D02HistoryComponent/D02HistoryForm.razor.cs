...
{
	public partial class D02HistoryForm
	{
		#region Service
		...
		#region Field
		public D02HistoryModel row = new();
		#endregion
		...
		{
			Loading.Show();
			row = await D02HistoryService.GetRowByID(ID) ?? new();
		...
		private async void OnSubmit()
		{
			...
			if (ID == null)
			{
				var res = await D02HistoryService.Insert(row);

				Loading.Close();

				if (res?.Data != null)
				{
					NavigationManager.NavigateTo($"/inquiry/datadebiturbadanusahahistory/{FormTransactionHistoryID}/debiturhistory02/{res.Data.ID}", true);
				}
			}
			#endregion
			...
			{
				await D02HistoryService.UpdateByID(row);
			}
			#endregion
		...
		private void Back()
		{
			NavigationManager.NavigateTo($"/inquiry/datadebiturbadanusahahistory/{FormTransactionHistoryID}");
		}
		#endregion
