...
namespace IFinancing360_SLIK_UI.Components.F01Component
{
	public partial class F01Form
	{
		...
		{
			Loading.Show();
			row = await F01Service.GetRowByID(ID) ?? new();
		...
		private async void OnSubmit()
		{
			...
			if (ID == null)
			{
				var res = await F01Service.Insert(row);

				Loading.Close();

				if (res?.Data != null)
				{
					NavigationManager.NavigateTo($"/transaction/datakredit/{FormTransactionID}/datakredit01/{res.Data.ID}", true);
				}
			}
			#endregion
			...
			{
				await F01Service.UpdateByID(row);
			}
			#endregion
		...
		private void Back()
		{
			NavigationManager.NavigateTo($"/transaction/datakredit/{FormTransactionID}");
		}
		#endregion
