...
namespace Data.Service
{
	...
	public class SysGeneralCodeService
	{
		...
		// Constructor
		public SysGeneralCodeService(IFINSLIKClient ifinslikClient)
		{
		...
		public async Task<List<SysGeneralCodeModel>?> GetRows(string keyword, int offset, int limit)
		{
			...
			// Get List Data
			var res = await _ifinslikClient.GetRows<SysGeneralCodeModel>(_apiController, _apiRouteGetRows, new { Keyword = keyword, Offset = offset, Limit = limit });
		...
		public async Task<SysGeneralCodeModel?> GetRowByID(string? id)
		{
			...
			// Get Single Data
			var res = await _ifinslikClient.GetRow<SysGeneralCodeModel>(_apiController, _apiRouteGetRowByID, parameters);
		...
		public async Task<BodyResponse<object>> ChangeStatus(SysGeneralCodeModel model)
		{
