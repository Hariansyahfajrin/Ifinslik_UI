...
namespace Data.Service
{
	...
	public class FormTransactionService
	{
		...
		// API Controller
		public readonly string _apiController = "FormTransaction";
		...
		// Constructor
		public FormTransactionService(IFINSLIKClient ifinslikClient)
		{
			_ifinslikClient = ifinslikClient;
		}

		public async Task<List<FormTransactionModel>?> GetRows(string keyword, int offset, int limit, string formType, DateTime? date)
		{
			// var parameters = new
			// {
			// 	Keyword = keyword,
			...
			// Get List Data
			var res = await _ifinslikClient.GetRows<FormTransactionModel>(_apiController, _apiRouteGetRows, new { Keyword = keyword, Offset = offset, Limit = limit, FormType = formType, Date = date?.ToString("yyyy-MM-dd") });

			return res?.Data;
		}
		public async Task<FormTransactionModel?> GetRowByID(string? id)
		{
			...
			// Get Single Data
			var res = await _ifinslikClient.GetRow<FormTransactionModel>(_apiController, _apiRouteGetRowByID, parameters);

			return res?.Data;
		}
		public async Task<BodyResponse<BaseModel>> Insert(FormTransactionModel model)
		{
			// Mengirim model ke API Insert dengan HTTP Post
			var res = await _ifinslikClient.Post(_apiController, _apiRouteInsert, model);
			return res;
		}
		public async Task<BodyResponse<object>> UpdateByID(FormTransactionModel model)
		{
		...
		// public async Task<BodyResponse<object>> ChangeStatus(FormTransactionModel model)
		// {
