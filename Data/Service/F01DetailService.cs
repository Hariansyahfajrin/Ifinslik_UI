...
namespace Data.Service
{
	...
	public class F01Service
	{
		...
		// API Controller
		public readonly string _apiController = "F01";
		...
		// Constructor
		public F01Service(IFINSLIKClient ifinslikClient)
		{
			_ifinslikClient = ifinslikClient;
		}

		public async Task<List<F01Model>?> GetRows(string keyword, int offset, int limit, string formTransactionID)
		{
			// var parameters = new
			// {
			// 	Keyword = keyword,
			...
			// Get List Data
			var res = await _ifinslikClient.GetRows<F01Model>(_apiController, _apiRouteGetRows, new { Keyword = keyword, Offset = offset, Limit = limit, formTransactionID });

			return res?.Data;
		}
		public async Task<F01Model?> GetRowByID(string? id)
		{
			...
			// Get Single Data
			var res = await _ifinslikClient.GetRow<F01Model>(_apiController, _apiRouteGetRowByID, parameters);

			return res?.Data;
		}
		public async Task<BodyResponse<BaseModel>> Insert(F01Model model)
		{
			// Mengirim model ke API Insert dengan HTTP Post
			var res = await _ifinslikClient.Post(_apiController, _apiRouteInsert, model);
			return res;
		}
		public async Task<BodyResponse<object>> UpdateByID(F01Model model)
		{
		...
		public async Task<List<F01Model>> GetRowsForLookup(string keyword, int offset, int limit, string? formTransactionId)
		{
			var res = await _ifinslikClient.GetRows<F01Model>(_apiController, _routeGetRowsForLookup, new { keyword, offset, limit, formTransactionId });
			return res.Data;
		}
