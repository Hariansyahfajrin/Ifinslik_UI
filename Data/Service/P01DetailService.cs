...
namespace Data.Service
{
	...
	public class P01Service
	{
		...
		// API Controller
		public readonly string _apiController = "P01";
		...
		// Constructor
		public P01Service(IFINSLIKClient ifinslikClient)
		{
			_ifinslikClient = ifinslikClient;
		}

		public async Task<List<P01Model>?> GetRows(string keyword, int offset, int limit, string formTransactionID)
		{
			// var parameters = new
			// {
			// 	Keyword = keyword,
			...
			// Get List Data
			var res = await _ifinslikClient.GetRows<P01Model>(_apiController, _apiRouteGetRows, new { Keyword = keyword, Offset = offset, Limit = limit, formTransactionID });

			return res?.Data;
		}
		public async Task<P01Model?> GetRowByID(string? id)
		{
			...
			// Get Single Data
			var res = await _ifinslikClient.GetRow<P01Model>(_apiController, _apiRouteGetRowByID, parameters);

			return res?.Data;
		}
		public async Task<BodyResponse<BaseModel>> Insert(P01Model model)
		{
			// Mengirim model ke API Insert dengan HTTP Post
			var res = await _ifinslikClient.Post(_apiController, _apiRouteInsert, model);
			return res;
		}
		public async Task<BodyResponse<object>> UpdateByID(P01Model model)
		{
